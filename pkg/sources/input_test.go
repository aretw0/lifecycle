...
func TestInputSource_WithBackoff(t *testing.T) {
	...
	src := sources.NewInputSource(
		sources.WithInputReader(strings.NewReader("q\n")),
		sources.WithInputBackoff(10*time.Millisecond),
	)
	...
	select {
	case ev := <-src.Events():
		if _, ok := ev.(control.ShutdownEvent); !ok {
...
func TestInputSource_UnknownHandler(t *testing.T) {
	...
	handled := false
	src := sources.NewInputSource(
		sources.WithInputReader(r),
		sources.WithUnknownHandler(func(cmd string, known []string) {
			if cmd != "unknown_cmd" {
